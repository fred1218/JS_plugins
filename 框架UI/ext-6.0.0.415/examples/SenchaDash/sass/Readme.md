...
folder are mapped to the application's root namespace, 'SenchaDash'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SenchaDash"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SenchaDash' in the same way
as `"SenchaDash/sass/src"`.
